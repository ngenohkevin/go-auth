...
run:
	docker run --name ${APP_NAME} --network ${NETWORK} -p 8080:8080 -e GIN_MODE=release -e DB_SOURCE="postgresql://root:secret@go-auth-db:5432/go-auth?sslmode=disable" ${APP_NAME}:latest
